...
The enquiry_form model also uses the foreign key of the CustomUser in a one-to-many capacity. The budget was programmed to be optional. When a user selects a country, this is from a prepopulated list of selectable countries stored in a constant.py file of tuples approved by the site owner.
...
![Admin view](documentation/readme/admin-view.png)

The Admin site is extremely important for the site-owner in this project as it has been customised for ease of use and searchability. It is where the Site Owner creates blog, views comments and queries, uploads/updates/deletes products, appoints moderators and more.
